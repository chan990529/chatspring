...
function StockManagement() {
    ...
    // 이름으로 검색
    const searchStocks = () => {
        axios.get(`http://localhost:8080/api/stocks/search?name=${searchTerm}`)
    ...
    const filterByDate = () => {
        ...
        axios.get(`http://localhost:8080/api/stocks/filter?startDate=${start}&endDate=${end}`)
            .then(response => setStocks(response.data))
            .catch(error => console.error(error));
    };
    ...
    const handleSubmit = (e) => {
        ...
        if (isDuplicateStock(formattedStock.stockName, formattedStock.capturePrice, formattedStock.captureDate)) {
            ...
            axios.post('http://localhost:8080/api/stocks', formattedStock)
    ...
    const deleteSelectedStocks = () => {
        axios.post('http://localhost:8080/api/stocks/delete', { ids: selectedStocks })
            .then(() => {
    ...
    const handleTextSubmit = (e) => {
        ...
        if (nonDuplicateStocks.length > 0) {
            axios.post('http://localhost:8080/api/stocks/batch', nonDuplicateStocks)
                .then(response => {
